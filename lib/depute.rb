...
nom_depute.map{|m|  arr_nom_depute << m.text; puts m.text}
...
def perform
    ...
    puts  get_email(m)  #afficher chaque email du deputé 
}
    return email
end 
...
i= 0
while (i<arr_nom_depute.length) do
    h_arr << {"first_name" => prenom[i], "last_name" => nom[i],"email" =>email[i] }
    puts "first_name : #{ prenom[i]} ,last_name : #{nom[i]}, email : #{email[i]}"
i+=1
